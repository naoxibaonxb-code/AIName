  ...
  <view class="page"><view class="safe" />
    ...
    <view class="content">
      ...
      <view class="card">
        ...
        <view class="field"><text class="label">邮箱</text><input v-model="profileForm.email" class="input readonly" type="text" disabled /></view>
...
const records = ref([]), loginCount = ref(0)
const savingProfile = ref(false), savingPassword = ref(false), cancelling = ref(false)
...
async function saveProfile() {
  ...
  try {
    const updated = await api.updateProfile({ username: profileForm.username })
...
@media(min-width:768px){.content{padding:34px 28px 70px}.identity{padding-left:8px;padding-right:8px}.stats{padding:24px 12px}.card{padding:30px 34px}.field,.primary{max-width:720px}.record{padding:18px 0}}
</style>
